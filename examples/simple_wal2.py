"""
A specification of a simple database log. This second version recovers the
database by replaying the synced prefix of the log after a restart.
...
class KvStoreState(MachineStateBase):
    ...
    LOG_BOUND: Param[
        int
    ]  # the upper bound on the log size (to prevent unbounded growth)
...
def kv_from_synced_log(s: KvStoreState) -> Expr:
    """Replay the durable prefix of the log into a key/value map."""
    return s.log[: s.log_synced].reduce(  # type: ignore[misc]
        apply_command, s.KEYS.map_to(lambda _: Val(0))
    )
...
def kv_step(c: Context[KvStoreState]):
    ...
    with start:
        ...
        # The process has just started or restarted after a crash. Recover the
        # database by replaying only the synced prefix of the log.
        s.kv_mem = kv_from_synced_log(s)
        s.log = s.log[: s.log_synced]  # type: ignore[misc]
        s.pc = Val(PC.Wait)
    with set_:
...
def step(c: Context[KvStoreState]):
    ...
    with crash:
        ...
        # Only the synced prefix of the log is durable.
        s = c.state
        s.log = s.log[: s.log_synced]  # type: ignore[misc]
        s.pc = Val(PC.Start)
        s.pending = Command.Nop()  # type: ignore[operator]
